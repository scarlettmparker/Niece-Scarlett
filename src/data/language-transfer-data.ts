...
const EMPTY_FAQ: ResolvedFaq = { title: "", body: "", footer: "" };
...
defineLoader({
  ...
  async loader(params) {
    ...
    const result = await fetchPropertySet(
      OWNER_KEY,
      SET_NAME,
      `faq.${language}`
    );
    const entry = result.success
      ? (result.data?.gaiaQueries.propertySet as LanguageTransferEntry | null)
      : null;
    const faq = await resolveEntry(entry);

    if (!faq && language !== FALLBACK_LANGUAGE) {
      const fallback = await fetchPropertySet(
        OWNER_KEY,
        SET_NAME,
        `faq.${FALLBACK_LANGUAGE}`
      );
      const fallbackEntry = fallback.success
        ? (fallback.data?.gaiaQueries.propertySet as LanguageTransferEntry | null)
        : null;
 ...
 */
async function resolveEntry(entry: LanguageTransferEntry | null): Promise<ResolvedFaq | null> {
  if (!entry) {
    return null;
  }
  if (entry.kind === "blog") {
 ...
 */
async function resolveBlog(entry: LanguageTransferEntry): Promise<ResolvedFaq | null> {
  if (entry.remoteObject) {
    ...
    return postToFaq(
      (await fetchLocateBlogPost(id)).data?.blogQueries.locateBlogPost ?? null
    );
  }
  if (entry.typeName) {
 ...
 * @return the rendered FAQ, or null when the post is missing
 */
function postToFaq(post: { title: string; content?: string | null } | null): ResolvedFaq | null {
  if (!post) {
    return null;
  }
