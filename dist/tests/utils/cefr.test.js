...
describe("weightedLevelLabel", () => {
    ...
    it("keeps an adjacent bump within the band", () => {
        expect(weightedLevelLabel([p(CefrLevel.B2, 0.8), p(CefrLevel.C2, 0.2)])).toBe("mid B2");
    });
    ...
    it("labels a value just past a level as high of that level", () => {
        expect(weightedLevelLabel([p(CefrLevel.B1, 0.7), p(CefrLevel.B2, 0.3)])).toBe("high B1");
    });
