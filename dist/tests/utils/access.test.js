...
import { matchPermission, refundRateLimit, reserveRateLimit, } from "~/utils/access.js";
describe("matchPermission", () => {
    it("matches an exact permission", () => {
        expect(matchPermission("bot.commands.classify", "bot.commands.classify")).toBe(true);
    });
    ...
    it("matches a mid-path wildcard", () => {
        expect(matchPermission("bot.commands.classify", "bot.*.classify")).toBe(true);
    });
    it("rejects a non-matching permission", () => {
        expect(matchPermission("bot.commands.pronounce", "bot.commands.classify")).toBe(false);
        expect(matchPermission("bot.commands.pronounce", "bot.commands")).toBe(false);
        expect(matchPermission("other.thing", "bot.*")).toBe(false);
    });
    it("treats dots literally, not as regex wildcards", () => {
        expect(matchPermission("botXcommands.classify", "bot.commands.*")).toBe(false);
    });
});
