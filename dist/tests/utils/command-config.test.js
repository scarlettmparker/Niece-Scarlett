...
import { effectiveCommandConfig, flushCommandConfig, rateLimitFor, } from "~/utils/command-config.js";
...
describe("effectiveCommandConfig", () => {
    ...
    it("lets the propertyset override a code default", async () => {
        ...
        const config = await effectiveCommandConfig(command({ permission: "bot.default" }));
    ...
    it("falls back to the code default when the entry has no value", async () => {
        ...
        const config = await effectiveCommandConfig(command({ rateLimit: { capacity: 1, refillPerSecond: 0.1 } }));
...
describe("rateLimitFor", () => {
    ...
    it("uses a channel override when present", () => {
        expect(rateLimitFor({ rateLimit: { ...globalRate, channels: { ch1: override } } }, "ch1")).toEqual(override);
    });
