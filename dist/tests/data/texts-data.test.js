...
describe("texts loader", () => {
    ...
    it("maps a query spec into pagination input", async () => {
        ...
        await resolvePageData("texts", "texts", { spec });
        expect(vi.mocked(executeDocument)).toHaveBeenCalledWith(ListTextsDocument, {
            pagination: {
                page: 1,
                size: 10,
                sortBy: "title",
    ...
    it("defaults page and sort when the spec has neither", async () => {
        await resolvePageData("texts", "texts", {
            spec: { page: 0, filters: [] },
        });
        const call = vi.mocked(executeDocument).mock.calls[0][1];
