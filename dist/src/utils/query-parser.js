 ...
 */
function collectPhrases(schema, excludeWords) {
    ...
    for (const field of Object.values(schema.fields))
        add(field.aliases);
    for (const sortField of Object.values(schema.sort?.fields ?? {}))
        add(sortField.aliases);
    for (const words of Object.values(OPERATOR_WORDS))
        add(words);
    for (const words of Object.values(DIRECTION_WORDS))
        add(words);
    for (const words of Object.values(schema.sort?.directionWords ?? {}))
        add(words);
 ...
 */
function isStopWord(token, excludeSet) {
 ...
 */
function isControlWord(token, ctx) {
    return (ctx.pageAliases.has(token) ||
 ...
 */
function parseFilter(tokens, valueStart, fieldName, explicitOperator, config, ctx, spec) {
    ...
    while (j < tokens.length) {
        ...
        if (isStopWord(token, ctx.excludeSet) || isControlWord(token, ctx))
            break;
        if (config.values) {
 ...
 */
function parseSearch(tokens, valueStart, searchTokens, ctx) {
    let j = valueStart;
 ...
 */
function parseSort(tokens, start, schema, ctx, spec) {
    let j = start;
    if (tokens[j] === "by" || tokens[j] === "on")
        j++;
    let field;
    let direction;
 ...
 * @param excludeWords the intent trigger words to drop from free text
 */
export function parseQuerySpec(utterance, schema, excludeWords) {
    const phrases = collectPhrases(schema, excludeWords);
    let normalized = ` ${utterance.toLowerCase()} `;
    for (const phrase of phrases) {
        normalized = normalized.replace(new RegExp(`\\b${escapeRegExp(phrase)}\\b`, "g"), key(phrase));
    }
    const tokens = normalized.trim().split(/\s+/).filter(Boolean);
    const spec = { page: 0, filters: [], search: undefined };
    const searchTokens = [];
    const pageAliases = new Set((schema.pageAliases ?? DEFAULT_PAGE_ALIASES).map(key));
    const fieldAliasMap = new Map();
    for (const [fieldName, field] of Object.entries(schema.fields)) {
        for (const alias of field.aliases)
            fieldAliasMap.set(key(alias), fieldName);
    }
