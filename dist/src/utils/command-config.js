 ...
 */
export async function effectiveCommandConfig(command) {
    const entry = await fetchConfig(command.name);
    const rateLimit = mergeRateLimit(command.rateLimit, entry.rateLimit);
    return {
        permission: entry.permission === undefined ? command.permission : entry.permission,
        rateLimit,
    };
}
/**
 ...
 */
export function rateLimitFor(config, channelId) {
 ...
 */
export function flushCommandConfig() {
    CACHE.clear();
}
/**
 ...
 */
function mergeRateLimit(code, overrides) {
