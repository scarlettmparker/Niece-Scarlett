 ...
 */
export function parseDefineScope(text) {
 ...
 */
export function splitDefineArgs(args, excludeWords = []) {
...
function defineEmbed(def) {
    ...
    if (def.compounds.length > 0) {
        ...
        for (const compound of def.compounds) {
            ...
            lines.push(`- ${compound.term} *(${compound.wordType})* -> ${translations}`);
        }
    }
    if (def.relatedWords.length > 0) {
        lines.push("", `**Related**: ${def.relatedWords.map((w) => w.term).join(", ")}`);
    }
...
const command = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option
        .setName("scope")
        .setDescription("Comma-separated: examples, all translations, compounds, related")),
    async messageExecute(message, args, intent) {
        const { word, scopeText } = splitDefineArgs(args, intent?.words ?? []);
        if (!word) {
            await message.reply("Usage: `ns define <word> [with examples, all translations, compounds, related]`");
            return false;
        }
