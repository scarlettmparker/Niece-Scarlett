...
import { fetchAccessibleCommands, fetchBlogPostsByRemoteObjects, } from "~/utils/api.js";
import { sendable } from "~/utils/sendable.js";
import { loadTranslations } from "~/translations/index.js";
import { helpListEmbed, helpDetailEmbed, } from "~/components/help-embed.js";
 ...
 */
async function accessibleCommands(discordId) {
    const result = await fetchAccessibleCommands(discordId, OWNER_KEY, PROPERTY_SET);
    if (!result.success || !result.data?.gaiaQueries)
        return [];
