 ...
 */
export function helpListEmbed(commands) {
 ...
 */
export function helpDetailEmbed(entry, helpContent) {
 ...
 */
function addDescription(embed, entry) {
 ...
 * @param entry the command entry
 */
function addPermission(embed, entry) {
 ...
 * @param entry the command entry
 */
function addRateLimit(embed, entry) {
 ...
 */
function addGlobalRateLimit(embed, rl) {
 ...
 * @param rl the rate limit config
 */
function addChannelRateLimits(embed, rl) {
 ...
 */
function formatRateLimit(capacity, refillPerSecond) {
    if (capacity == null || refillPerSecond == null)
        return null;
    const perSecond = Number(refillPerSecond);
    const interval = perSecond >= 1 ? `${perSecond}/s` : `${Math.round(1 / perSecond)}s`;
